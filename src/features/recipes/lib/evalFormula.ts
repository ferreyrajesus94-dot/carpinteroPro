...
function tokenize(input: string): Token[] {
  ...
  while (i < input.length) {
    ...
    if ((c >= '0' && c <= '9') || c === '.') {
      ...
      if (!Number.isFinite(n)) throw new Error(`Número inválido: ${input.slice(i, j)}`)
...
export function evalFormula(expr: string, vars: Record<string, number>): number {
  ...
  function parseFactor(): number {
    ...
    if (t.type === 'op' && t.value === '(') {
      ...
      const v = parseExpr()
      const close = consume()
      if (!close || close.type !== 'op' || close.value !== ')') throw new Error('Falta ")"')
      return v
    }
    if (t.type === 'num') { consume(); return t.value }
    if (t.type === 'ident') {
      consume()
      if (!(t.value in vars)) throw new Error(`Variable desconocida: ${t.value}`)
      return vars[t.value]
    }
