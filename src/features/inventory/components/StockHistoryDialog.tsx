  ...
  return (
    <div className="space-y-2">
      <p className="text-xs text-muted-foreground">
        Stock actual: <strong>{formatNum(material.stock)}</strong> {material.unit} ·{' '}
            ...
            <div key={m.id} className="flex items-start gap-3 p-3 text-sm">
              <div
                className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${
                  isIn ? 'bg-green-500/15 text-green-600' : 'bg-destructive/15 text-destructive'
                }`}
              >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between gap-2">
                  ...
                  <span className={`font-mono font-semibold ${isIn ? 'text-green-600' : 'text-destructive'}`}>
                    {isIn ? '+' : ''}{formatNum(m.delta)}
                  </span>
                </div>
                <p className="text-xs text-muted-foreground">
