...
interface ClientDetailProps {
	/** Pre-computed quote sale prices and metadata for the current client */
	quotesWithSalePrice: ClientDetailQuoteItem[];
	/** Pre-computed per-client stats: {count, total} */
	statsByClient: Record<string, { count: number; total: number }>;
	/** Whether quote data is still loading and destructive actions must wait */
...
export function ClientDetail({
	...
	const clientQuotes = quotesWithSalePrice;
	const quotesByClient = statsByClient;
			...
			<div className="pb-24 md:pb-6 p-4 md:p-6">
				<div className="md:grid md:grid-cols-[340px_1fr] md:gap-4">
					...
					<aside className="hidden md:flex flex-col rounded-xl border border-line bg-surface overflow-hidden max-h-[calc(100dvh-3rem)]">
						...
						<div className="overflow-y-auto divide-y divide-[var(--line)]">
							...
							{filteredClients.length === 0 && (
								<p className="p-4 text-center text-[12px] text-ink3">
									Sin resultados.
								</p>
							)}
						</div>
					...
					<div className="space-y-4">
							...
							<div className="flex items-start gap-4">
								...
								<div className="flex gap-2">
									...
									<Button
										variant="outline"
										size="sm"
										disabled={isQuotesLoading || clientQuotes.length > 0}
						...
						{/* Presupuestos */}
						<section
							className={`${tab !== "presupuestos" ? "hidden md:block" : ""}`}
						>
							<div className="rounded-xl border border-line bg-surface p-4 md:p-5">
								<h2 className="font-display text-[14px] font-semibold text-ink mb-3">
											...
											>
												<div className="min-w-0">
													<div className="flex items-center gap-2">
														...
														<QuoteStatusBadgeSlot status={q.status} />
													</div>
						...
						<section className={`${tab !== "notas" ? "hidden md:block" : ""}`}>
							<div className="rounded-xl border border-line bg-surface p-4 md:p-5">
								...
								{client.notes ? (
									<p className="whitespace-pre-wrap text-sm text-ink2">
										{client.notes}
									</p>
								) : (
