...
export function QuoteForm() {
  ...
  function canAdvance(currentStep: number): boolean {
    switch (currentStep) {
      case 1: return Boolean(clientIdWatch)
      case 2: return Boolean(furnitureNameWatch) && (recipeCostWatch ?? 0) > 0
      case 3: return true
      case 4: return true
    ...
    <div className="fixed inset-0 bg-background flex flex-col overflow-hidden z-50">
      ...
      <div className="flex items-center justify-between border-b border-line bg-surface px-4 py-3 sm:px-6 sm:py-4">
        <div>
          ...
          {isEditing && (existingQuote?.recipe_snapshots?.length ?? 0) > 0 && (
            <span className="inline-flex text-xs font-medium text-blue-600 mt-1">Versión congelada</span>
          )}
        </div>
        <Button
          variant="ghost"
      ...
      <div className="border-b border-line bg-cp-bg2 px-4 py-3 sm:px-6">
        <div className="flex gap-2 sm:gap-4 max-w-full overflow-x-auto">
          {STEPS.map((s) => (
            ...
            >
              <div className={`w-7 h-7 rounded-full grid place-items-center font-mono text-xs ${
                step === s.n ? 'bg-cp-accent text-white' :
                s.n < step ? 'bg-cp-accent/20 text-cp-accent' :
                'bg-line text-ink2'
              }`}>
                {s.n}
              </div>
          ...
          <div className="max-w-2xl space-y-6">
            ...
            {step === 1 && (
              <div className="space-y-4">
                <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">¿Para quién es?</h2>
                <p className="text-sm text-muted-foreground">Elegí un cliente existente o creá uno nuevo al vuelo.</p>
            ...
            {step === 2 && (
              <div className="space-y-4">
                <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">¿Qué vas a hacer?</h2>
                <p className="text-sm text-muted-foreground">Elegí una plantilla o creá uno desde cero.</p>

                {templates.length > 0 && (
                  <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-2">Plantillas</label>
                      ...
                      {templates.map((t) => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            const cost = computeRecipeCost(t.recipe_items, t.labor_items)
                            setValue('recipe_cost', cost.total)
                          }}
                        ...
                        >
                          ...
                          <p className="font-medium text-xs line-clamp-2">{t.name}</p>
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            {step === 3 && (
              <div className="space-y-4">
                <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Costos extra</h2>
                <p className="text-sm text-muted-foreground">Flete, instalación, herrajes, lo que se te cobra aparte.</p>
            ...
            {step === 4 && (
              <div className="space-y-6">
                <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Margen y precio final</h2>
                ...
                <div className="space-y-3">
                  <Label className="text-xs font-semibold uppercase tracking-wide">Estado inicial</Label>
                ...
                <div className="space-y-3">
                  <Label htmlFor="notes" className="text-xs font-semibold uppercase tracking-wide">Notas internas (opcional)</Label>
                  <Textarea
                    id="notes"
                    {...register('notes')}
                    rows={3}
      ...
      <div className="border-t border-line bg-surface px-4 py-3 sm:px-6 flex flex-col-reverse sm:flex-row gap-2 sm:gap-4 sm:items-center sm:justify-between">
        ...
        <div className="lg:hidden text-center sm:text-left">
          ...
          <p className="font-display text-xl font-semibold">${(salePrice || 0).toLocaleString('es-AR')}</p>
        </div>
