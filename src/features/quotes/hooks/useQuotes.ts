...
import {
  ...
  deleteQuote,
  generateQuoteNumber,
  maybeAutoDiscountStock,
} from '../api/quotes'
import type { QuoteInsert, QuoteUpdate, QuoteExtraInsert } from '../types'
import type { RecipeSnapshotInput, LaborSnapshotInput, PieceSnapshotInput } from '../api/quotes'
...
export function useCreateQuote(workshopId: string) {
  ...
  return useMutation({
    mutationFn: ({ quote, extras, recipeSnapshots, laborSnapshots, pieceSnapshots }: CreatePayload) =>
      createQuote(quote, extras, recipeSnapshots, laborSnapshots, pieceSnapshots),
    onSuccess: () => {
...
export function useUpdateQuote(workshopId: string) {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async ({ id, quote, extras, recipeSnapshots, laborSnapshots, pieceSnapshots }: UpdatePayload) => {
      ...
      return updateQuote(id, quote, extras, recipeSnapshots, laborSnapshots, pieceSnapshots)
    },
