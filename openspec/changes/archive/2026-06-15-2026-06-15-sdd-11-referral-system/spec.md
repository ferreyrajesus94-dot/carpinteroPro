# Delta Spec — SDD-11 Referral System

> **Composite spec for change `2026-06-15-sdd-11-referral-system`.**
> Covers one **new capability** (`referral-program`) and two **modified capabilities**
> (`sdd-2-billing-mercadopago`, `sdd-3-auth-profile-hardening`).
> Mirrors the proposal's Capabilities section exactly.
> See `proposal.md` (intent, scope, risks) and `explore.md` (current-state map, decisions) for context.

---

# Part 1 — NEW Capability: `referral-program`

This is a **full spec** (no existing behavior to reference). Eight domains cover schema, attribution, discount, commission, admin APIs, admin UI, tenant UI, and security.
...
# Part 2 — DELTA for `sdd-2-billing-mercadopago`

These are **MODIFIED** requirements that change the existing spec. Per archive rules, the full requirement block is included (unchanged scenarios preserved + new/updated scenarios).
...
# Part 3 — DELTA for `sdd-3-auth-profile-hardening`

These are **ADDED** requirements (existing behavior is preserved; new behavior is purely additive on top of the existing `handle_new_user` trigger).
...
| **executive_summary** | Wrote one new full spec (`referral-program`, 8 domains) and two deltas (`sdd-2-billing-mercadopago`, `sdd-3-auth-profile-hardening`) covering schema/RLS, signup attribution, first-period discount, webhook commission recording, admin Edge Functions, admin UI, tenant UI, and security. All requirements use RFC 2119 keywords and Given/When/Then scenarios. Key invariants enforced: self-referral blocked at trigger layer, webhook idempotency via unique `provider_payment_id`, RLS on platform-global tables with no `authenticated` policies, first-period discount only (no mutation of preapproval after creation), and commission from first payment including discounted amount. |
| **artifacts** | `openspec/changes/2026-06-15-sdd-11-referral-system/spec.md` |
| **next_recommended** | `design` — select chained PRs (already forecast as 5 WUs in proposal), define work-unit boundaries, pick tests-first approach. |
| **risks** | Spec is dense; design phase should slice into 5 chained PRs under the 400-line budget. The `handle_new_user` change touches an existing trigger — design must keep the change additive. Webhook idempotency relies on a unique index that must be added in the same migration as the table. Self-referral prevention requires the trigger to look up `youtubers.contact_email` via a join — must stay within SECURITY DEFINER privileges. |
| **skill_resolution** | `paths-injected` (sdd-spec SKILL.md loaded by orchestrator) |
