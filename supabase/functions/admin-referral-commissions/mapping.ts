...
export function validateCommissionsRequest(
		...
		if (typeof body.youtuberId !== "string" || !body.youtuberId.trim()) {
			return {
				...
				error: {
					...
					message: "youtuberId must be a non-empty string",
				},
			};
		}
	...
	if (body.toDate !== undefined) {
		if (typeof body.toDate !== "string" || !body.toDate.trim()) {
			return {
				ok: false,
				error: { code: "validation_error", message: "toDate must be a string" },
			};
		}
...
export function buildCommissionJsonResponse(rows: CommissionRow[]): {
